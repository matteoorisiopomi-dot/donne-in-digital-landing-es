...
**Vocabulario:** peninsular — *móvil* (no *celular*), *ordenador* (no
*computadora*), *vídeo* con tilde, *dependienta / peluquera /
administrativa*.
...
- **Sin reclamos absolutos:** nada de *garantizado*, *100 %*, *seguro*.
...
| --- | --- | --- |
| Navbar → `aria-label` del enlace al logo | *(marca IT)* *(`brand.name`)* | `${BRAND_NAME}` *(interpolado)* |
| Navbar → `alt` de la imagen del logo | *(marca IT)* *(`brand.name`)* | `${BRAND_NAME}` *(interpolado)* |
...
modo de mirar (*con atención*) y `kw-rose` la promesa real (*trabajar
online desde casa*, coherente con `labels.goal`). En el subtítulo
`kw-urgent` envuelve la escasez (*no siga disponible mucho tiempo*). Las clases CSS son idénticas al original: solo
...
| `<iframe title>` (×2: vimeo y youtube) | Video VSL | Vídeo de presentación *(alineado con el `aria-label` de la sección: "VSL" es jerga interna)* |
...
| Cifras de la cuenta atrás | `MM:SS` | `MM:SS` *(sin cambios)* |
...
| Apellidos | placeholder | Es. Rossi | p. ej. García Ruiz *(dos apellidos, coherente con la etiqueta en plural)* |
| Email | label | Email | Email |
| Email | placeholder | nome@esempio.it | nombre@ejemplo.es |
| Teléfono | label | CONTATTO WHATSAPP | Número de WhatsApp *(sentence case, como el resto de etiquetas)* |
| Teléfono | `aria-label` del selector de prefijo | Prefisso internazionale del telefono | Prefijo internacional del teléfono |
| Teléfono | opción del selector | 🇨🇭 Svizzera (+41) · 🇩🇪 Germania (+49) · 🇪🇸 Spagna (+34) | 🇪🇸 España (+34) *(único prefijo admitido)* |
...
| *fallback en el script* | Numero non valido. | Número no válido. |

> Solo se aceptan **móviles** españoles: `6xxxxxxxx` y `71x`–`79x`
> (9 dígitos). Los fijos `8xx` / `9xx` y el rango `70x` (*numeración
> personal*, que no es un móvil) se rechazan a propósito, en coherencia
...
| `labels.consent.privacyText` (enlace bajo la casilla) | *(no existía en el original)* | Consulta nuestra Política de privacidad. |
...
| Posición | Texto italiano original | Traducción ES |
| --- | --- | --- |
| `<title>` | Grazie — *(marca IT)* | Gracias — `${BRAND_NAME}` |
...
*coach por WhatsApp*. Se ha corregido la grafía de la marca: *Whatsapp* →
**WhatsApp**.
...
| Posición | Texto italiano original | Traducción ES |
| --- | --- | --- |
| `footer.tagline` | Comunicazione digitale per chi vuole cambiare lavoro. | Comunicación digital para quien quiere cambiar de trabajo. |
| `footer.copyright` | © *(año)* *(marca IT)*. Tutti i diritti riservati. | © *(año)* `${BRAND_NAME}`. Todos los derechos reservados. |
...
| `footer.legalLinks[0].label` | *(no existía en el original)* | Aviso legal → `/aviso-legal` |
| `footer.legalLinks[1].label` | *(no existía en el original)* | Política de privacidad → `/privacidad` |
...
| --- | --- | --- |
| Cabecera estándar (`aware = yes`) | 🔔 Nuovo lead — *(marca IT)* | 🔔 Nuevo lead ES - `${BRAND_NAME}` |
...
| Etiqueta 6 | Perché proprio adesso | Por qué justo ahora *(etiqueta interna de Slack, no visible al lead)* |
...
| Valor sí | ✅ Sì | ✅ Sí |
| Valor no | ❌ No | ❌ No |
| Bloque de contexto final | Ricevuto *(ISO timestamp)* | Recibido: *(ISO timestamp)* |
...
| Posición | Texto italiano original | Traducción ES |
| --- | --- | --- |
| `meta.title` (home) | *(marca IT)* — Lavora da remoto gestendo la comunicazione online | `${BRAND_NAME}` — Trabaja en remoto gestionando la comunicación online |
| `meta.description` (home) | Impara a gestire chat, messaggi e presenza online delle aziende. Da casa. Anche partendo da zero. | Aprende a gestionar los chats, los mensajes y la presencia online de pequeñas empresas. Desde casa. Aunque empieces de cero. |
| `<html lang>` | `it` | `es` |
| `og:locale` | `it_IT` | `es_ES` |
| `site.webmanifest` → `name` | *(marca IT)* | `${BRAND_NAME}` *(generado)* |
| `site.webmanifest` → `short_name` | *(marca IT, forma corta)* | derivado de `${BRAND_NAME}` *(palabras ≤ 2 caracteres descartadas)* |
| `site.webmanifest` → `lang` | *(ausente)* | `es` |
| `robots.txt` → comentarios | *(en italiano)* | *(traducidos al castellano)* |
...
1. **Tuteo consistente.** Toda la landing tutea. La única forma verbal en
   segunda persona del plural es *"que me contactéis"* en el texto de
   consentimiento y *"pedírnosla"* en el aviso legal: se refieren al
   equipo (vosotros), no a la usuaria. Es correcto en castellano
   peninsular y deliberado.
2. **Género.** El público objetivo son mujeres: *"¿Invertirías en ti
   misma?"*, *"una coach"*, *"dependienta, peluquera, administrativa"*.
...
5. **Palabras prohibidas** que NO deben aparecer nunca: *certificación*,
   *certificación oficial*, *garantizado*, *100 %*, *seguro* (como
